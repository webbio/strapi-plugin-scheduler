...
export default ({ strapi }: { strapi: Strapi }) => ({
	...
	async update(id, data) {
		return await strapi.entityService.update('plugin::scheduler.scheduler', id, {
			data
		});
	},
	async findOne(id) {
	...
	async schedule(data) {
		...
		const existingArchiveEntry = existingEntries.find((entry) => entry.type === 'archive');
	...
	async getByUidAndEntryId(uid, entryId) {
		const result = await strapi.query('plugin::scheduler.scheduler').findMany({ where: { uid, entryId } });
