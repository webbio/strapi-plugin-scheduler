...
export default ({ strapi }: { strapi: Strapi }) => ({
	...
	async schedule(data) {
		...
		if (existingPublishEntry && !data?.publishAt) {
			return await this.delete(existingPublishEntry.id);
		}

		if (existingArchiveEntry && !data?.archiveAt) {
			return await this.delete(existingArchiveEntry.id);
		}
